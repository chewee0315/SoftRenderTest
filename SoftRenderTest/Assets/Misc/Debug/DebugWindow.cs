...
public class dbwin {
	...
	public class WindowBase {
		...
		{
			this.title    = title;
			this.win_rect = new Rect(20, 20, 200, 500);
			this.id       = id;
...
public class DebugWindow : MonoBehaviour {
	...
	void	Update()
	{
		...
		InputManager.get().ClearOcuppyRegionAll(InputManager.Layer.Debug);

		Rect	rect = new Rect();

		rect.position = new Vector3(10.0f, 0.0f);
		rect.size     = new Vector2(300.0f, 20.0f);

		InputManager.get().AddOcuppyRegion(InputManager.Layer.Debug, rect);

		if(this.active_window != null) {

			rect = this.active_window.window.win_rect;
...
//#if UNITY_EDITOR
	...
	{
		this.setActiveWindow(this.console);
	}
